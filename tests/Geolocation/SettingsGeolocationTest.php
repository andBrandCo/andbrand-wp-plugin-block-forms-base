...
test('Register method will call custom hooks', function () {
	...
	$this->assertSame(10, \has_filter(SettingsGeolocation::FILTER_SETTINGS_SIDEBAR_NAME, 'SebFormsWpPlugin\Geolocation\SettingsGeolocation->getSettingsSidebar()'), 'The callback getSettingsSidebar should be hooked to custom filter hook with priority 10.');
	$this->assertSame(10, \has_filter(SettingsGeolocation::FILTER_SETTINGS_GLOBAL_NAME, 'SebFormsWpPlugin\Geolocation\SettingsGeolocation->getSettingsGlobalData()'), 'The callback getSettingsGlobalData should be hooked to custom filter hook with priority 10.');
	$this->assertSame(10, \has_filter(SettingsGeolocation::FILTER_SETTINGS_GLOBAL_IS_VALID_NAME, 'SebFormsWpPlugin\Geolocation\SettingsGeolocation->isSettingsGlobalValid()'), 'The callback isSettingsGlobalValid should be hooked to custom filter hook with priority 10.');
});
